    ...
    def wrapped(*args, **kwargs):
        ...
        for webhook_url in WebhookUrls.query.all():
            if webhook_url.enabled:
                current_app.logger.debug("Sending event to: {}".format(webhook_url.url))
                event = {
                    "event": g.event_type,
                    "timestamp": int(time.time()),
                    "software_title": g.event["id"],
                    "version:": g.event["currentVersion"],
                    "url": url_for("jamf_pro.patch_by_name_id", name_id=g.event["id"]),
                }
                ...
                current_app.logger.info(
                    "The webhook is not enabled for: " "{}".format(webhook_url.url)
                )
